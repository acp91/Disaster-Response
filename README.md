...
* precision, defined as <img src="https://latex.codecogs.com/svg.latex?\Large&space;x=\frac{\text{TP}}{\text{TP  FP}}" title="\Large x=\frac{\text{TP}}{\text{TP + FP}}" />, is good for measuring your model's performance when the cost of False Positive is high. False positive in our case would be message classified as "Accident" when in fact it isn't
